...
public final class Exercises {
	...
	public static void main(String[] args) {
		...
		String tx = 
				...
				"430600";
		InputStream txStream = new ByteArrayInputStream(tx.getBytes());
		Transaction txn = Transaction.parse(InputSource.wrap(txStream));
		
		List<Input> inputs = txn.getInputs();
		System.out.println("Total number of inputs = "+inputs.size());
		int i = 1;
		for (Input input:inputs) {
			if (i == 2) {
				System.out.println("The ScriptSig of 2nd input = "+input.getScriptSig());
				break;
			}
		...
		List<Output> outputs = txn.getOutputs();
		System.out.println("Total number of outputs = "+outputs.size());
		i = 1;
		for (Output output:outputs) {
			if (i == 1) {
				System.out.println("The ScriptPubKey of 1st output = "+output.getScriptPubKey());
			}
			
			if (i == 2) {
				System.out.println("The Amount of 2nd output = "+output.getAmount());
				break;
			}
